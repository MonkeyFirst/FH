...
// Обработка событий с гильзами в анимации пулемёта
void Character::HandleAnimationTrigger(StringHash eventType, VariantMap& eventData)
{
	...
	if (as)
	{
		...
		Quaternion quatGuns = nodeGuns_->GetWorldRotation(); // ориентация всего корпуса пушек
...
void CharacterFireWithAutoGun::Update(float timeStep) 
{
	...
	if (t >= l) // если это конец анимации с выстрелами сбросим время и переключим стейт перса
	{
...
void CharacterFireWithAutoGun::EnterState() // call once then new state are set 
{
	...
	// в анимации со стрельбой активируем кость с которой связан эффект огня от выстрела
...
{
	// при выходе из этой анимации в другие спрячем сетку огня
